...
import {initialPilot,leaveAircraft,boardAircraft,canBoard,boardingPoint,exitReason,stepPilot,idlePilotInput,airportObstacles,inLounge} from '../lib/flight/pilot.ts';
...
assert.ok(boardAircraft(p,s));assert.equal(p.active,false);assert.equal(s.brake,false);assert.ok(p.walked>200);
