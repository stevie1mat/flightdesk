...
for(const label of plane.group.getObjectByName('Boeing livery lettering').children){const pos=label.geometry.attributes.position;const side=Math.sign(pos.getX(0)),tail=label.name==='737 tail marking';const ray=new THREE.Raycaster(new THREE.Vector3(side*5,tail?3.25:-.52,tail?15.3:-9.7),new THREE.Vector3(-side,0,0),0,10);const base=tail?plane.group.getObjectByName('vertical stabilizer'):shell;const hits=ray.intersectObjects([label,base],false);assert.ok(hits.length&&hits[0].object===label,`${label.name}: livery buried in shell`);}
