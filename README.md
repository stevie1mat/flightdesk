...
## Pilot life / free roam

Press **V** or **Leave aircraft** while stopped on the ground with the throttle at idle and gear down. The aircraft is parked while you explore. Use **WASD** to walk, **Shift** to run, drag the scene to look around, or use the arrow keys. Gamepad sticks also move/look; the primary button runs. **P** pauses exploration. Walk to the boarding stairs beside the forward left door and press **E** (or Board aircraft) to return to the cockpit; the parking brake is released on boarding.

Both airports have an open pilot lounge, seating, passing crew, a pedestrian path and night lighting. Walk through the lounge entrance to rest until morning or night. The local map tracks your position and heading, the parked aircraft, and the lounge. Follow the causeway toward the city to explore farther. Walking checks the shoreline, building footprints and fuselage; terrain height follows the existing scenery. The arrival report also offers an option to leave and explore Everett. Graphics still default to Performance.

This is the initial single-player free-roam layer: first-person movement and lounge interactions within the existing world. It does not yet simulate jobs, conversations, homes, vehicles you can drive, or a saved career. Crew are ambient animated characters. Boarding uses an interaction at the stairs rather than a cabin/stair-climbing animation. Run `node --experimental-strip-types tests/pilot.mjs` for the walk-to-lounge-and-return loop, boarding, flight resumption and collision checks.

Airport people now use textured, skinned male and female traveler models from the MIT-licensed Microsoft Rocketbox library. The local 2K face/body/hair maps and joint-based walk cycle replace the primitive figures. Sole contact is baked across the gait; path heading follows the direction of travel, and soft contact shadows work in Performance mode. Shared geometry/textures, grouped material draws, and distance culling reduce crowd cost. Asset sources and license are in `public/assets/people/CREDITS.md`. Run `node tests/people.mjs` to check skeleton independence, proportions, knee animation and sole height across a complete stride.

## Third-person pilot and walkable aircraft

On foot, **C** toggles **Third person** (default) and **First person**. The textured pilot avatar follows your movement with walking/standing animation; drag or use arrow keys to look/turn. The follow camera shortens around buildings, the airframe, cabin walls and seat backs, and switches to a shorter distance inside.

At the forward left boarding point, **E** now enters the cabin rather than jumping directly into the flight seat. Walk the central aisle, explore the forward vestibule and cockpit, then press **E** near the captain's seat to sit and use the flight controls. Return to the forward door and press **E** to disembark. While seated and parked, **B** / **Walk cabin** stands you up inside; **V** / **Leave aircraft** still takes you outside. Walking requires a stopped aircraft, idle throttle and lowered gear; the aircraft stays parked during exploration.

The interior includes a carpeted aisle, passenger seating, overhead bins, cabin lighting, a forward galley, cockpit seats, yokes, throttle pedestal and live display textures. Fixed surfaces are grouped by material for rendering. It is a simplified game interior (including seating layout), not a dimensionally exact 737 cockpit/cabin. Cabin windows use lit glass panels; the flight-deck forward view looks onto the live world. Run `node --experimental-strip-types tests/interior.mjs` for the full exterior/cabin/seat/exit loop and camera clearance; `node tests/people.mjs` also checks the playable avatar.
