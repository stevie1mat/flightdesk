...
import {WalkingSurfaces} from './walking-surfaces';
...
function box(parent: THREE.Object3D, w: number, h: number, d: number, x: number, y: number, z: number, mat: THREE.Material) { const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), mat); m.position.set(x, y, z); m.castShadow = true; m.receiveShadow = true;if(h<=.2&&y<=.3)m.userData.walkingSurface=true; parent.add(m); return m; }
export class FlightRenderer {
    private floors=new WalkingSurfaces();private interior!:AircraftInterior;pilot: PilotState|null=null; private pilotWorld!:PilotWorld; private obstacles=airportObstacles(); private stopDrag=()=>{};
    walk(input:PilotInput,dt:number,s:FlightState){if(this.pilot&&!s.paused)stepPilot(this.pilot,input,dt,(x,z)=>{if(Math.abs(x)>19000||z>7000||z<-45000)return null;return this.floors.height(x,z,isLand(x,z)?groundHeight(x,z):null);},this.obstacles,s);}
