...
import { MorningScenery } from './scenery';
...
export class FlightRenderer {
    ...
    buildWorld() {
        this.scenery = new MorningScenery(this.scene, this.renderer, this.manager); const tarmac = this.scenery.asphalt, concrete = material(0xb3b7a9, .88), paint = material(0xf3f2e8, .8), yellow = material(0xf0c855);
    ...
    pointerDown = (e: PointerEvent) => { if (this.mode !== 'Free') return; const startX = e.clientX, startY = e.clientY, angle = this.freeAngle, height = this.freeHeight; const move = (ev: PointerEvent) => { this.freeAngle = angle + (ev.clientX - startX) * .007; this.freeHeight = Math.max(3, Math.min(1200, height + (ev.clientY - startY) * .3)); }; const up = () => { window.removeEventListener('pointermove', move); window.removeEventListener('pointerup', up); }; window.addEventListener('pointermove', move); window.addEventListener('pointerup', up); };
    ...
    dispose() { this.disposed = true; this.resizeObserver.disconnect(); this.container.removeEventListener('pointerdown', this.pointerDown); this.container.removeEventListener('wheel', this.wheel); this.scene.traverse(o => { if (o instanceof THREE.Mesh || o instanceof THREE.Points) { o.geometry.dispose(); const mats = Array.isArray(o.material) ? o.material : [o.material]; mats.forEach(m => { if ('map' in m) (m.map as THREE.Texture)?.dispose(); m.dispose(); }); } }); this.night.dispose(); this.hdSky?.dispose(); this.scenery.dispose(); this.environmentTarget?.dispose(); this.composer.passes.forEach(p => p.dispose()); this.composer.dispose(); this.renderer.dispose(); this.renderer.domElement.remove(); }
}
