...
export class FlightRenderer {
    pilot: PilotState|null=null; private pilotWorld!:PilotWorld; private obstacles=airportObstacles(); private stopDrag=()=>{};
    ...
    buildWorld() {
        this.pilotWorld=new PilotWorld(this.scene);
    ...
    render(s: FlightState, dt: number) {
        ...
        this.plane.update(s, dt); this.plane.setCockpitView(!this.pilot?.active&&this.mode === 'Cockpit');
        if(this.pilot?.active){const pilot=this.pilot;this.camera.position.set(pilot.x,pilot.y,pilot.z);this.camera.up.set(0,1,0);this.target.set(pilot.x+Math.sin(pilot.heading)*Math.cos(pilot.look),pilot.y+Math.sin(pilot.look),pilot.z-Math.cos(pilot.heading)*Math.cos(pilot.look));}else {const chase = this.mode === 'Chase'; if (this.mode === 'Cockpit') { this.desired.set(0, 1.10, -17.15); this.desired.applyQuaternion(p.quaternion).add(p.position); this.target.set(0, 1.10, -500).applyQuaternion(p.quaternion).add(p.position); this.camera.position.copy(this.desired); this.camera.up.set(0, 1, 0).applyQuaternion(p.quaternion); } else { const angle = chase ? 0 : this.freeAngle; this.desired.set(Math.sin(angle) * this.zoom, chase ? 23 : this.freeHeight, Math.cos(angle) * this.zoom); this.desired.applyAxisAngle(new THREE.Vector3(0, 1, 0), -s.heading).add(p.position); if (chase) this.camera.position.copy(this.desired); else this.camera.position.lerp(this.desired, 1 - Math.exp(-dt * 3)); this.target.copy(p.position).add(new THREE.Vector3(0, 3, 0)); this.camera.up.set(0, 1, 0); } } const shake=this.effects.update(s,dt,this.container.clientHeight*this.renderer.getPixelRatio());if(shake>0){this.camera.position.x+=Math.sin(this.visualTime*83)*shake;this.camera.position.y+=Math.cos(this.visualTime*97)*shake*.7;}this.camera.lookAt(this.target);
    ...
    dispose() { this.stopDrag();this.disposed = true; this.resizeObserver.disconnect(); this.container.removeEventListener('pointerdown', this.pointerDown); this.container.removeEventListener('wheel', this.wheel); this.scene.traverse(o => { if (o instanceof THREE.Mesh || o instanceof THREE.Points) { o.geometry.dispose(); const mats = Array.isArray(o.material) ? o.material : [o.material]; mats.forEach(m => { if ('map' in m) (m.map as THREE.Texture)?.dispose(); m.dispose(); }); } }); this.night.dispose(); this.hdSky?.dispose(); this.scenery.dispose(); this.environmentTarget?.dispose(); this.composer.passes.forEach(p => p.dispose()); this.composer.dispose(); this.renderer.dispose(); this.renderer.domElement.remove(); }
}
