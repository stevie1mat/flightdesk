...
export class MorningScenery {
  ...
  constructor(scene: THREE.Scene, renderer: THREE.WebGLRenderer, manager: THREE.LoadingManager) {
    ...
    for (const center of [0, -25000]) {
      ...
      const roadMat = new THREE.MeshStandardMaterial({ color: 0x666e6a, roughness: .95 }); const road = new THREE.Mesh(new THREE.BoxGeometry(1300, 2, 19), roadMat); road.position.set(1040, 1, center + 820); road.receiveShadow = true;road.userData.walkingSurface=true; scene.add(road);
    ...
    const streetMat = new THREE.MeshStandardMaterial({ color: 0x64716c, roughness: .9 }); for (let i = 1; i < 17; i++) { const street = new THREE.Mesh(new THREE.PlaneGeometry(16, 35000), streetMat); street.rotation.x = -Math.PI / 2; street.position.set(1645 + i * 310, .1, -13000); street.userData.walkingSurface=true;scene.add(street); } for (let i = 0; i < 110; i++) { const street = new THREE.Mesh(new THREE.PlaneGeometry(4800, 13), streetMat); street.rotation.x = -Math.PI / 2; street.position.set(4270, .1, 3455 - i * 310); street.userData.walkingSurface=true;scene.add(street); }
