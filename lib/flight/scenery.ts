...
function groundHeight(x: number, z: number) { const mainland = Math.max(0, x - 5500) / 11000; const western = Math.max(0, -x - 9000) / 9000; return Math.max(0, (noise(x, z) + .62) * Math.max(mainland, western) * 850); }
...
export class MorningScenery {
  ...
  constructor(scene: THREE.Scene, renderer: THREE.WebGLRenderer, manager: THREE.LoadingManager) {
    ...
    for (let row = 0; row < 110; row++)for (let col = 0; col < 16; col++) { const x = 1800 + col * 310 + (rand() - .5) * 90, z = 3300 - row * 310; if (x < coastline(z) + 120 || rand() < .14) continue; const downtown = Math.exp(-Math.pow((z + 3500) / 2300, 2)) * Math.exp(-Math.pow((x - 3300) / 1700, 2)); const h = 12 + rand() * 35 + Math.pow(rand(), 1.2) * downtown * 350; const w = 45 + rand() * 75, d = 45 + rand() * 80; dummy.position.set(x, h / 2 + groundHeight(x, z), z); dummy.scale.set(w, h, d); dummy.updateMatrix(); buildings.setMatrixAt(building, dummy.matrix); buildings.setColorAt(building, new THREE.Color().setHSL(.53 + rand() * .07, .05 + rand() * .08, .55 + rand() * .35)); dummy.position.y += h / 2 + 2; dummy.scale.set(w * .65, 4 + rand() * 6, d * .6); dummy.updateMatrix(); roofs.setMatrixAt(building, dummy.matrix); building++; } buildings.count = roofs.count = building; buildings.receiveShadow = true; scene.add(buildings, roofs);
